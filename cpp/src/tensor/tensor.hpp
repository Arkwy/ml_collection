...
class TensorBase {
  ...
  public:
    ...
    const vector<size_t> &get_stride() const { return stride; }
    ...
    static Derived zeros(const vector<size_t> &shape, const D &device = D()) { return This::full(shape, (T)0, device); }
    static Derived ones(const vector<size_t> &shape, const D &device = D()) { return This::full(shape, (T)1, device); }
    static Derived empty(const vector<size_t> &shape, const D &device = D()) { return Derived(shape, device); }
